...
from torch.amp import autocast, GradScaler          # FIX: non-deprecated AMP import
...
VAL_INTERVAL     = 2
BATCH_SIZE       = 1
SW_OVERLAP_VAL   = 0.25   # 0.5 only for final evaluation
PATIENCE         = 15
PRED_THRESHOLD   = 0.4
ACCUM_STEPS      = 4      # OOM: higher accumulation, lower peak memory
NUM_CROP_SAMPLES = 4      # OOM: was 8, halves activation memory per batch
ANEURYSM_REPEAT  = 5
REPLAY_PCT       = 0.2
EMA_DECAY        = 0.999

# ── z-spacing filters ─────────────────────────────────────────────────────────
# Val cases go through Spacingd resampling so we can be more lenient.
# Raise VAL_Z_MAX if one of your 3 val cases is being dropped.
TRAIN_Z_MAX = 2.0
VAL_Z_MAX   = 3.0
...
def tta_inference(model, inputs, roi_size, overlap):
    ...
    for axes in TTA_FLIPS:
        ...
        pred = sliding_window_inference(
            x, roi_size=roi_size, sw_batch_size=1,   # OOM: 2→1
            predictor=model, overlap=overlap,
        )
        if axes:
            pred = torch.flip(pred, axes)
        preds.append(torch.sigmoid(pred))
    print(f"    TTA: {len(preds)} flip augmentations averaged")
...
def main():
    ...
    # ── Data — separate z-filter thresholds for train vs val ──────────────────
    ...
    for i, s in zip(images, segs):
        ...
        z    = float(hdr.get_zooms()[2])

        if stem in ANEURYSM_VAL_P2:
            # FIX: relaxed z-filter for val — Spacingd resamples to 1mm anyway
            if z <= VAL_Z_MAX:
                val_files.append({"img": i, "seg": s})
            else:
                print(f"  DROPPED from val (z={z:.2f}mm > {VAL_Z_MAX}): {stem}")
        else:
            if z <= TRAIN_Z_MAX:
                train_files.append({"img": i, "seg": s})

    print(f"Usable train volumes: {len(train_files)}")
    print(f"Usable val volumes  : {len(val_files)}")

    # ── Report val case z-spacings for transparency ────────────────────────────
    print("Val cases:")
    for d in val_files:
        stem = get_stem(d["img"])
        z    = float(nib.load(d["img"]).header.get_zooms()[2])
        print(f"  {stem}  z={z:.3f}mm")

    # ── Build phase 2 splits ───────────────────────────────────────────────────
    aneurysm_train = [d for d in train_files
                      if get_stem(d["img"]) in ANEURYSM_TRAIN_P2]
    normal_cases   = [d for d in train_files if not is_aneurysm(d["img"])]
    aneurysm_val   = val_files   # already filtered above

    n_replay = int(len(normal_cases) * REPLAY_PCT)
    print(f"Aneurysm train : {len(aneurysm_train)} cases × {ANEURYSM_REPEAT} "
          f"repeat = {len(aneurysm_train)*ANEURYSM_REPEAT} entries/epoch")
    print(f"Vessel replay  : {n_replay}/{len(normal_cases)} cases, "
          f"re-sampled each epoch")
    print(f"Phase 2 val    : {len(aneurysm_val)} aneurysm cases")
    ...
    train_transforms = Compose(shared_pre + [
        RandCropByPosNegLabeld(
            ...
            pos=9, neg=1,
            num_samples=NUM_CROP_SAMPLES,   # OOM: 8→4
            allow_smaller=True,
        ),
        ...
        Rand3DElasticd(keys=["img", "seg"],
                       sigma_range=(5, 10), magnitude_range=(200, 500),
                       prob=0.7, mode=("bilinear", "nearest")),
        ...
        RandHistogramShiftd(keys=["img"], num_control_points=10, prob=0.3),
    ...
    # ── Val dataset (fixed, fully cached) ─────────────────────────────────────
    ...
    # Single scheduler — no Cosine conflict
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Re-sample replay cases each epoch for vessel variety
        ...
        if (epoch + 1) % VAL_INTERVAL == 0:
            ema.apply_shadow()
            model.eval()
            print(f"  TTA validation (EMA weights, {len(TTA_FLIPS)} flip augs)...")
            ...
            print(f"  per-case Dice: {[f'{d:.4f}' for d in per_case_dices]}  "
                  f"(min={min(per_case_dices):.4f})")

            # FIX: capture improvement BEFORE updating bests, so the
            # no_improve_count check isn't comparing updated values to themselves
            dice_improved = val_dice > best_metric
            hd95_improved = val_hd95 < best_hd95

            # Save while shadow weights are still active (single apply_shadow above)
            if dice_improved:
            ...
            # Single restore after all saves
            ema.restore()

            # FIX: increment only when NEITHER metric improved
            if dice_improved or hd95_improved:
                no_improve_count = 0
            else:
                no_improve_count += 1
        ...
        if no_improve_count >= PATIENCE:
            print(f"\nEarly stopping — {PATIENCE} val checks without improvement")
            break
