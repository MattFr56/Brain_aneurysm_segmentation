...
class ProjectionLayer(nn.Module):
    def __init__(self, in_channels, dim=6, dimensions='2D', norm=nn.GroupNorm):
        ...
        if dimensions == '2D':
            ...
            raise ValueError("dimensions should be '2D' or '3D'")  # FIX 2
...
class MoV2D(nn.Module):
    ...
    def forward_self(self, q, k, num_heads):
        ...
        """
        B, C, H, W = q.shape
        q = q.reshape(B, C // num_heads, num_heads, H, W)
        k = k.reshape(B, C // num_heads, num_heads, H, W)
        ...
        attn = torch.cat(attn, dim=1)                           # (B, kernel_size**2, num_heads, H, W)
        attn = self.softmax(attn)[:, :, np.newaxis, :, :, :]   # (B, kernel_size**2, 1, num_heads, H, W)

        v  = self.makeV(num_heads)                              # (1, kernel_size**2, 2, num_heads, 1, 1)
        v_ = torch.sum(attn * v, dim=1)                        # (B, 2, num_heads, H, W)

        x_v = torch.mean(v_, dim=2)                            # (B, 2, H, W)
        return x_v

    def forward(self, x_q, x_k, num_heads):
        return self.forward_self(x_q, x_k, num_heads)
...
class MoV3D(nn.Module):
    ...
    def makeV(self, num_heads):
        v = self.grid.reshape((self.kernel_size) ** 3, 3)[
    ...
    def forward_self(self, q, k, num_heads):
        """
        Args:
            q: query features (B, C, H, W, D)
            k: key features   (B, C, H, W, D)
        """
        B, C, H, W, D = q.shape
        q = q.reshape(B, C // num_heads, num_heads, H, W, D)
        k = k.reshape(B, C // num_heads, num_heads, H, W, D)
        ...
        for i in range(self.kernel_size):
            for j in range(self.kernel_size):
                for s in range(self.kernel_size):
                    mask_k_idx = torch.where(
                        mask_k[:, :, :, i:i + H, j:j + W, s:s + D] < 0.5
                    )
                    attn_i = torch.sum(
                        q * k[:, :, :, i:i + H, j:j + W, s:s + D], dim=1, keepdim=True
                    )
                    attn_i[mask_k_idx] = -1000.
                    attn.append(attn_i)   # (B, 1, num_heads, H, W, D)

        attn = torch.cat(attn, dim=1)                                # (B, kernel_size**3, num_heads, H, W, D)
        attn = self.softmax(attn)[:, :, np.newaxis, :, :, :, :]     # (B, kernel_size**3, 1, num_heads, H, W, D)

        v  = self.makeV(num_heads)                                   # (1, kernel_size**3, 3, num_heads, 1, 1, 1)
        v_ = torch.sum(attn * v, dim=1)                             # (B, 3, num_heads, H, W, D)

        x_v = torch.mean(v_, dim=2)                                 # (B, 3, H, W, D)
        return x_v
...
class COVER(nn.Module):
    def __init__(self,
        ...
        for i in range(depth):
            self.proj.append(
                ProjectionLayer(chan[len(chan) // 2 + i], head_dim * num_heads[i], dimensions=dimensions)
            )

        self.stn = SpatialTransformer()

    def VPA(self, M, F):
        M = list(M)  # FIX 3: work on a shallow copy — don't mutate the input list
