...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, Conv):
        super().__init__()
        # FIX 2: max(1, ...) prevents 0 groups if out_channels < 8
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, Conv, mode):
        super().__init__()
        # FIX 3: align_corners required for bilinear/trilinear, not supported for nearest
        align_corners = True if mode in ('bilinear', 'trilinear') else None
...
class UNet_base(nn.Module):
    def __init__(self, n_channels,
        ...
        if dimensions == '2D':
            ...
            raise ValueError("dimensions should be '2D' or '3D'")  # FIX 1
