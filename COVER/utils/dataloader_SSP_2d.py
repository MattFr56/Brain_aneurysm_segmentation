    ...
    for fname in os.listdir(file_dir):
        if is_image_file(fname):
            ...
            for s in range(vol.shape[2]):
                ...
                # Génération du masque vasculaire
                vessel_mask = generate_vessel_mask(slice_img)
                # Sauvegarde
                out_name = fname.replace('.nii.gz', '').replace('.nii', '') + f'_slice{s:03d}.npy'
                np.save(join(output_dir, out_name), slice_img)
                np.save(join(output_dir, f"mask_{out_name}"), vessel_mask.astype(np.float32))

def generate_vessel_mask(slice_img):
    vesselness = frangi(slice_img, sigmas=range(1, 3))  # Filtre de Frangi pour les vaisseaux
    vesselness = (vesselness - vesselness.min()) / (vesselness.max() - vesselness.min())  # Normalisation [0,1]
    return vesselness > 0.5  # Seuil pour binariser
...
class DatasetFromFolder2D(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = np.load(self.filenames[index].replace('.npy', '_mask.npy'))  # Charge le masque
        img = torch.from_numpy(img).unsqueeze(0)  # Ajoute la dimension channel
        mask = torch.from_numpy(mask).unsqueeze(0)
        return img, mask  # Retourne aussi le masque
